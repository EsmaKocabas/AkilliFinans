...
if __name__ == "__main__":
    musteri_path = sys.argv[1] if len(sys.argv) > 1 else 'src/analytics/veri/demand_points.csv'
    atm_path = sys.argv[2] if len(sys.argv) > 2 else 'src/analytics/veri/atm_candidates.csv'
    kume_sayisi = int(sys.argv[3]) if len(sys.argv) > 3 else 5
    print(json.dumps(optimize_atm_lokasyonlari_v2(musteri_path, atm_path, kume_sayisi), indent=4))
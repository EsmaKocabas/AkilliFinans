...
export class PythonService {
  private static SCRIPT_PATH = path.resolve(process.cwd(), 'src/analytics/wlp_motoru.py');
  private static MUSTERI_CSV = path.resolve(process.cwd(), 'src/analytics/veri/demand_points.csv');
  private static ATM_CSV = path.resolve(process.cwd(), 'src/analytics/veri/atm_candidates.csv');
   ...
   */
  private static async syncDbToCsvFiles(): Promise<void> {
  ...
  public static async runAtmOptimization(kumeSayisi: number = 5): Promise<any> {

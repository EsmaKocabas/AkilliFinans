...
export class TransactionController {
  ...
  public static async createTransaction(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized', message: 'User not authenticated.' });
      ...
      const insertRes = await db.query(
        `INSERT INTO transactions (user_id, title, category, amount, merchant, payment_method, date, reference_code)
         ...
         RETURNING *;`,
        [userId, title.trim(), category.trim(), txAmount, merchant.trim(), paymentMethod || 'Kart', dateStr, refCode]
      );

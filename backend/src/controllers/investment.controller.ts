...
export class InvestmentController {
  public static async getPortfolio(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
  ...
  public static getSuggestions(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
  ...
  public static async executeTrade(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized', message: 'User not authenticated.' });
      ...
      if (isBuy) {
        ...
        await db.query(
          `INSERT INTO investments (user_id, asset_type, symbol, amount)
           ...
           DO UPDATE SET amount = investments.amount + $4;`,
          [userId, assetType, symbol.toUpperCase(), tradeCost]
        );
      } else {
        // Selling: Check if user owns enough of this asset
        const invRes = await db.query(
          "SELECT amount FROM investments WHERE user_id = $1 AND UPPER(symbol) = $2;",
        ...
        const deductRes = await db.query(
          "UPDATE investments SET amount = amount - $1 WHERE user_id = $2 AND UPPER(symbol) = $3 RETURNING amount;",
          [tradeCost, userId, symbol.toUpperCase()]
        );

        const newAmount = parseFloat(deductRes.rows[0].amount);
        if (newAmount <= 0) {
          await db.query(
            "DELETE FROM investments WHERE user_id = $1 AND UPPER(symbol) = $2;",
            [userId, symbol.toUpperCase()]
          );
        }
      }
      ...
      await db.query(
        `INSERT INTO transactions (user_id, title, category, amount, merchant, payment_method, date, reference_code)
         VALUES ($1, $2, 'Yatırım', $3, 'Akıllı Yatırım Portföyü', 'Hesaptan Bakiye', $4, $5);`,
        [userId, `${symbol.toUpperCase()} ${isBuy ? 'Alımı' : 'Satımı'}`, isBuy ? -tradeCost : tradeCost, dateStr, refCode]
      );

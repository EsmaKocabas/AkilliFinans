...
export class NotificationController {
  public static async getNotifications(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const notifsRes = await db.query(
        "SELECT id, title, message, type, is_read as \"isRead\", created_at as \"createdAt\" FROM notifications WHERE user_id = $1 ORDER BY id DESC;",
        [userId]
      );

...
export class DbService {
    ...
    static getPath(envVar, fallback) {
        const relativePath = process.env[envVar] || fallback;
        return path.resolve(process.cwd(), relativePath);
    }
    static USERS_PATH = DbService.getPath('USERS_CSV_PATH', 'src/analytics/veri/users.csv');
    static DEMAND_POINTS_PATH = DbService.getPath('DEMAND_POINTS_CSV_PATH', 'src/analytics/veri/demand_points.csv');
    static ATM_CANDIDATES_PATH = DbService.getPath('ATM_CANDIDATES_CSV_PATH', 'src/analytics/veri/atm_candidates.csv');
    static TRANSACTIONS_PATH = path.resolve(process.cwd(), 'src/analytics/veri/transactions.csv');
    static INVESTMENTS_PATH = path.resolve(process.cwd(), 'src/analytics/veri/investments.csv');
    ...
    // Helper to write CSV
    static writeCSV(filePath, headers, rows) {
    ...
    // --- SEEDERS ---
    static seedDefaultTransactions() {
        const defaults = [
            {
                id: 1,
                user_id: 1,
